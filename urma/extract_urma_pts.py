    ...
    try:
        ...
        with open(f"./{year}/akurma_{date_str}.csv", "w") as csvfile:
            ...
            if len(hourly_files) != 24:
                ...
                logging.info(
                    f"Hour(s) missing from {date_s3_path}: {sorted(expected_hours - actual_hours)}"
                )
            ...
            for hour in hourly_files:
                ...
                with gdal.Open("/vsis3/" + hour) as ds:
                    ...
                    if ds.RasterCount < expected_band_count:
                        logging.info(
                            f"Band count ({ds.RasterCount}) is less than expected count ({expected_band_count}) for {hour}"
                        )
                        ...
                        try:
                            if var_codes[band.GetMetadata()["GRIB_COMMENT"]]:
                                for key, value in transformed_airport_dict.items():
                                    pixel_value = extract_point_value(
                                        band, value[2], value[3], transform=False
                                    )
                                    writer.writerow(
                                        [
                                            date_str,
        ...
        print(
            f"Error processing {args.date_s3_path}:\n{traceback.format_exc()}",
            flush=True,
        )
        raise
...
if __name__ == "__main__":
    ...
    args_list = [
        ProcessDateArgs(date_s3_path, transformer, transformed_airport_dict)
        for date_s3_path in date_s3_paths
    ]
